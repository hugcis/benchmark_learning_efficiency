...
def compute_ranks(x):
    """Returns ranks in [0, len(x)) Note: This is different from
    scipy.stats.rankdata, which returns ranks in [1, len(x)].
...
class CMAES:
    ...
    def result(
        self,
    ):  # return best params so far, along with historically best reward, curr
        # reward, sigma
        r = self.es.result
        return (r[0], -r[1], -r[1], r[6])


def mate(a: Tuple[np.ndarray, np.ndarray], b: Tuple[np.ndarray, np.ndarray]):
    c = (a[0][:], a[1][:])

    idx = np.where(np.random.rand((c[0].size)) > 0.5)
    c[0][idx] = b[0][idx]

    idx = np.where(np.random.rand((c[1].size)) > 0.5)
    c[1][idx] = b[1][idx]
    return c
...
class SimpleGA:
    ...
    def __init__(
        ...
        self.num_params = num_params
        self.num_bin_params = num_bin_params
        self.sigma_init = sigma_init
        ...
        self.elite_params = [
            (
                np.zeros(self.num_bin_params).astype(bool),
                np.zeros(self.num_params),
            )
            for _ in range(self.elite_popsize)
        ]
        self.elite_rewards = np.zeros(self.elite_popsize)
        self._best_param = np.zeros(self.num_params)
        self.best_reward = 0
    ...
    def ask(self):
        ...
        self.epsilon = [
            (
                (np.random.random(size=(self.num_bin_params)) < self.sigma),
                np.random.randn(self.num_params) * self.sigma,
            )
        ...
        for i in range(self.popsize):
            ...
            child_params = (
                np.bitwise_xor(child_params[0], self.epsilon[i][0]),
                child_params[1] + self.epsilon[i][1],
            )
    ...
    def tell(self, reward_table_result: np.ndarray):
        ...
        if self.weight_decay > 0:
            l2_decay = compute_weight_decay(
                self.weight_decay, [i[1] for i in self.solutions]
            )
            reward_table += l2_decay
        ...
        self.elite_params = [solution[i] for i in idx]
        ...
        if self.first_iteration or (self.curr_best_reward > self.best_reward):
            ...
            self._best_param = np.copy(self.elite_params[0])
...
class OpenES:
    ...
    def tell(self, reward_table_result):
        ...
        _ = self.optimizer.update(-change_mu)
...
class PEPG:
    ...
    def __init__(
        ...
        # option to use greedy es method to select next mu, rather than using
        # drift param
        self.elite_ratio = elite_ratio
        self.elite_popsize = int(self.popsize * self.elite_ratio)
    ...
    def tell(self, reward_table_result):
        ...
        if self.use_elite:
            ...
            self.optimizer.stepsize = self.learning_rate
            _ = self.optimizer.update(-change_mu)  # adam, rmsprop, momentum, etc.
